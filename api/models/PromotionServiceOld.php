...
class PromotionServiceOld extends Component
{
    ...
    public static function PromotionPrice($products, $prices, $code = NULL, $delivery_model, $payment_form = NULL, $likeIt = NULL)
    {
        ...
        if (!empty($products) && is_array($products) && sizeof($products) && !empty($prices) && is_array($prices) && sizeof($prices) && !empty($cart)) {
            ...
            if (!empty($cart_data['CartItems'])) {
                ...
                $calc_data = @file_get_contents($url, false, $context);
                ...
                if (!empty($calc_data)) {
                    LogFoHelper::log($url, $opts, $logRequestTime, $calc_data, $logResponseTime);
                    $calc_data = json_decode($calc_data);

                    if (!empty($calc_data)) {
                        if (!empty($calc_data->CartItems)){
                            foreach ($calc_data->CartItems as $cartItem) {
                                if(isset($products_ids[$cartItem->SKU])){
                                    if(ServiceProductHelper::isProductService($products_ids[$cartItem->SKU])){ //заново пересчитываем цены на услуги
                                        ...
                                        $total_sum += $price * $cartItem->Count;
                                    }else {
                                        $final_prices[$products_ids[$cartItem->SKU]] = $cartItem->FinalPrice;
                                        $total_sum += $cartItem->FinalPrice * $cartItem->Count;
                                    }
                        ...
                        if(isset($products_ids[$cartItem->SKU]) && !empty($calc_data->PayTypeId)){
                            $data['payFormId'][$products_ids[$cartItem->SKU]] = $calc_data->PayFormId;
                        }

                        if(!empty($calc_data->Shop)){
                            foreach ($calc_data->Shop as $value){
                                $data ['location_shop_code'][] = $value->LocationCode;
                            }
                        }

                        if (!empty($calc_data->Delivery)) {
                            foreach ($calc_data->Delivery as $delivery) {
                        ...
                        if (!empty($calc_data->Promocode)) {
                            if(!empty($calc_data->Promocode->IsPromocodeUsed) && $calc_data->Promocode->IsPromocodeUsed == true){
                                $data ['code'] = $calc_data->Promocode->PromocodeNumber;
                            }

                        }

                        if (!empty($calc_data->BonusSpent)) {
                            $data ['likeIt'] = $calc_data->BonusSpent;
                        }
                    }
                }
            }
